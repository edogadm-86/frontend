...
import { Bell, Search, LogOut, Globe, Settings, Moon, Sun, HelpCircle, Download } from 'lucide-react';
...
interface NotificationItem {
  ...
  route?: string; // optional: where to navigate when clicked
  read?: boolean; // <-- added so we can show unread badge & update UI
}
...
export const Header: React.FC<HeaderProps> = ({ user, onLogout, currentView }) => {
  ...
  // Close menus when clicking outside (but not when clicking inside)
  useEffect(() => {
  ...
  /*const handleSearch = (e: React.FormEvent) => {
    ...
    const found: string[] = [];
    if (query.includes('dog') || query.includes('pet')) found.push('Dogs');
    if (query.includes('health') || query.includes('vaccination') || query.includes('vet')) found.push('Health');
    if (query.includes('appointment') || query.includes('calendar') || query.includes('schedule')) found.push('Calendar');
    if (query.includes('training') || query.includes('command')) found.push('Training');
    if (query.includes('community') || query.includes('post') || query.includes('event')) found.push('Community');

    if (found.length) {
      alert(`Found results in: ${found.join(', ')}`);
    } else {
  ...
  const handleExportData = () => {
    const exportData = {
      ...
      note:
        'This is a basic export. Full implementation would include all dog data, health records, appointments, etc.',
    };
  ...
  const markNotificationAsRead = async (notificationId: string) => {
    try {
      ...
      // revert if failed
      revertReadOptimistic(notificationId);
    }
  };

  // Fallback route mapping if a notification doesn't have a `route`
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2 lg:space-x-4 flex-shrink-0">
            ...
            <div className="relative" ref={langMenuRef}>
              ...
              {showLanguageMenu && (
                <div
                  className="absolute right-0 mt-2 w-36 bg-white/95 dark:bg-gray-800/95 backdrop-blur-md rounded-xl shadow-2xl border border-white/30 dark:border-gray-700/30 py-2 z-[60]"
            ...
            <div className="relative hidden lg:block" ref={notifMenuRef}>
              ...
              {showNotifications && (
                <div
                  className="absolute right-0 mt-2 w-80 bg-white/95 dark:bg-gray-800/95 backdrop-blur-md rounded-xl shadow-2xl border border-white/30 dark:border-gray-700/30 py-2 z-[60] max-h-96 overflow-y-auto"
                  onClick={(e) => e.stopPropagation()}
                >
                    ...
                    <div className="py-2">
                      {notifications.map((notification) => (
                        <div
                          ...
                          onClick={async (e) => {
                            e.stopPropagation();
                            // optimistic read + persist
                            await markNotificationAsRead(notification.id);
                            const nextRoute =
                              notification.route ||
                              (notification.type ? routeFromType[notification.type] : 'dashboard');
                  ...
                  <div className="px-4 py-2 border-t border-gray-200 dark:border-gray-700 flex items-center">
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        loadNotifications();
            ...
            <div className="relative" ref={userMenuRef}>
              ...
              {showUserMenu && (
                <div
                  className="absolute right-0 mt-2 w-48 bg-white/95 dark:bg-gray-800/95 backdrop-blur-md rounded-xl shadow-2xl border border-white/30 dark:border-gray-700/30 py-2 z-[70]"
                  onClick={(e) => e.stopPropagation()}
                >
